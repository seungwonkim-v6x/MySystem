...
**No autonomous git-mutation grants (ADR-0018).** `/ship` is the only skill that commits or pushes, and it does so under explicit user approval. The one prior carve-out — `/ai-review-loop`'s bounded `review-loop(rN):` commits under ADR-0012 — was retired with the skill. A future skill wanting autonomous git mutation needs its own ADR and a measured budget gate; the retired one is a precedent for the *shape* of such a grant, not for granting one.
