...
The model proposes actions; the harness validates, authorizes, executes, records, and returns observations. RTK token compression via PreToolUse hook is an example. The v0.35.0 defense-in-depth hooks (secret-scanner, dangerous-command-blocker, env-file-protection, block-dangerous-git) are another. **Prefer native Claude Code features over custom workarounds** — if Anthropic ships `.claude/rules/`, use it; do not invent a parallel system.
