...
- [ ] **Audit non-final `[[ ]]` assertions in bats suites** — bats does not fail a test when a non-final `[[ ]]` returns false (verified 1.13.0), so mid-test assertions may be silently unenforced; sweep tests/*.bats and either move assertions last, chain `|| false`, or adopt bats-assert. Effort: S → S. P2. Trigger: found while fixing the test-72 CI failure (2026-07-13). **Second sighting 2026-08-09 (v0.59.1)**: a brand-new `INSTALL_LOCK_BUSY` assertion was written unchained and passed against the exact bug it was written to catch. The three new lock cases are chained; the rest of the suite is not. Raise to P1 on a third sighting. **Scoped 2026-08-12 (v1.0.0)**: probed on bats 1.13.0 — a non-final `[ ]` DOES fail the test, at top level and inside a `for` loop; only the `[[ ]]` keyword form silently passes. So the sweep is just the `[[ ]]` occurrences (scripts 7, codex-parity 54, hooks 17, model-overrides 9, step1-routing 3), not every mid-test assertion. The v1.0.0 gate suites use `[ ]` throughout and are unaffected.
