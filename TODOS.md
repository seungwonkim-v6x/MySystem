...
- [ ] **`render-codex-agents.sh --check` exits 0 on failure** — it prints `FAIL CONTRACT_HOOK_REGISTRATION_INVALID` but returns exit 0, so `--check` cannot be used as a gate by exit code; `tests/scripts.bats` test "generated projections are current" only catches it because it also matches on `PROJECTIONS_CURRENT` in the output. A `--check` that fails while exiting 0 will silently pass any future consumer that trusts `$?`. Effort: S → S. P2. Trigger: found while baselining the projection test during the deep-research vendor-facts change (2026-07-27).
