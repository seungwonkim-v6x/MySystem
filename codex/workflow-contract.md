<!-- Codex-only workflow contract.
     Claude Code does NOT read this file — it reads the thin ~/.claude/CLAUDE.md.
...
|------|------------------------|--------|
| 1. Frame the request | `/scope-check` (default) | user-owned |
|    (new idea) | `/office-hours` | gstack |
...
Codex core filesystem skills: `/scope-check`, `/office-hours`, `/investigate`, `/deep-research`, `/autoplan`, `/verify-test`, `/qa-only`, `/design-review`, `/verification-before-completion`, `/review`, `/requesting-code-review`, `/ship`, `/aside-qa`.
...
|---|---|
| 1 (`/scope-check`, `/office-hours`, or `/investigate`) | 2 (`/deep-research`) |
...
- `.claude/rules/operating-principles.md` — First Principle (User Outcome Over Existing Code), Boil the Lake, Harness Not Model, Vertical-Slice TDD, Conditional Clarification, Repo Mode, See Something Say Something
