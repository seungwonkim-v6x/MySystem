...
- `tests/model-overrides.bats` — 12 hermetic contract tests (sandbox repo via `MYSYSTEM_REPO_ROOT`, CI-safe): wrapper generation for file- and dir-symlink installs, model pin + preserved frontmatter + source marker, byte-identical idempotency, skip paths, stale-wrapper removal, relative-symlink canonicalization, stranded-temp sweep, tracked-skill pins, setup.sh delegation.
