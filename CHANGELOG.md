...
- `skills/deep-research/SKILL.md`: the `free_tier` column stated request quotas for credit-billed providers (`1000/mo` for exa, `~1000/mo` for firecrawl). Every metered row now points at the vendor's pricing page instead of naming a figure, and the column's contract is stated explicitly: it names where the allowance lives, never what it is. The two unmetered rows keep their descriptors (`plan-metered`, `local/OSS, no quota`). Restating an allowance out of this table into a report is exactly what the new rail forbids.
