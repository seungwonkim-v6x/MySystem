...
- `skills/deep-research/SKILL.md`: the `free_tier` column stated request quotas for credit-billed providers (`1000/mo` for exa, `~1000/mo` for firecrawl). The three credit-billed rows (exa, apify, firecrawl) now point at the vendor's pricing page instead of naming a figure, and the column's contract is stated explicitly: it names where the allowance lives, never what it is. The other three say why there is no page to point at — `plan-metered` (builtin, metered against the Claude plan), `local/OSS, no quota` (scrapling), and `no key, no allowance page` (context7). Restating an allowance out of this table into a report is exactly what the new rail forbids.
